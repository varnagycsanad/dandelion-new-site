...
- `AUT-003` `[ ]` Feladat: offer data schema definiálása a meglévő accommodation adatok referálásával; Függőség: `AUT-002`, `CNT-002`, `CNT-003`; Várható fájlok vagy rendszer: új offer data fájl(ok), meglévő data references; Elfogadási feltétel: nincs kézzel duplikált szállásadat, csak referenciák es kampányspecifikus mezők; Megjegyzés: ez a legfontosabb anti-duplication lépés, és a ténylegesen elérhető, már meglévő házlistát kell beleemelni a két kampány offer data-jába.
- `AUT-004` `[ ]` Feladat: közös offer sablon komponens megtervezése; Függőség: `AUT-003`; Várható fájlok vagy rendszer: várhatóan új Astro komponens vagy page template; Elfogadási feltétel: ugyanaz a sablon kiszolgálja a Fügeház es D2 kampányt eltérő tartalommal; Megjegyzés: a szerkezet a referencia landing logikáját követi: hero, célcsoport-blokk, szolgáltatások, programok, képi szekció, direkt foglalási előnyök, hangsúlyos CTA.
- `AUT-005` `[ ]` Feladat: közös szolgáltatásblokk es közvetlen foglalási előnyök blokk struktúrájának meghatározása; Függőség: `CNT-002`, `CNT-003`; Várható fájlok vagy rendszer: offer template, offer data; Elfogadási feltétel: a két landing ugyanazt a blokklogikát használja; Megjegyzés: az állítások igazoltsági szintje külön mezőként kezelendő, és a direkt foglalási előnyök blokk vizuálisan is külön, erős konverziós szekció legyen.
...
- `WEB-001` `[ ]` Feladat: Fügeház landing oldal implementációs specifikációja `Oszi kettesben a Szent Gyorgy-hegyen` munkacímmel; Függőség: `AUT-003`, `AUT-004`, `CNT-002`; Várható fájlok vagy rendszer: tervezett route `/ajanlatok/oszi-kettesben/`, offer data, offer template; Elfogadási feltétel: a landing tartalmaz hero-t, célcsoportot, rövid bemutatást, valós szolgáltatásokat, programblokkot, képi blokkot, közvetlen foglalási előnyöket es elsődleges CTA-t; Megjegyzés: a CTA a meglévő Fügeház Sabee linket használja, és a szerkezet a referencia páros landing ritmusát követi.
...
- `WEB-004` `[ ]` Feladat: D2 landing oldal implementációs specifikációja `Oszi napok egyutt a Balaton-felvideken` munkacímmel; Függőség: `AUT-003`, `AUT-004`, `CNT-003`; Várható fájlok vagy rendszer: tervezett route `/ajanlatok/oszi-csaladi-pihenes/`, offer data, offer template; Elfogadási feltétel: a landing tartalmaz hero-t, családi fókuszt, rövid bemutatást, valós szolgáltatásokat, programblokkot, képi blokkot, közvetlen foglalási előnyöket es elsődleges CTA-t; Megjegyzés: a dézsa nem jelenhet meg, a CTA a meglévő D2 Sabee linket használja, és a szerkezet a referencia családi landing logikáját követi.
...
- `WEB-008` `[ ]` Feladat: kétkártyás kampányblokk technikai specifikációja; Függőség: `WEB-001`, `WEB-004`, `AUT-006`; Várható fájlok vagy rendszer: főoldali komponens vagy section, image registry; Elfogadási feltétel: desktopon egymás mellett, mobilon egymás alatt jelenik meg a két ajánlatkártya; Megjegyzés: D1 kizárása explicit követelmény, a blokk a főoldalon kampány-belépési pontként működjön, ne általános szállásajánlóként.
