...
const clientPath = process.env.GEO_GSC_OAUTH_CLIENT_JSON;
const tokenPath = process.env.GEO_GSC_OAUTH_TOKEN_JSON;

if (!clientPath || !tokenPath) {
  console.error("HIÁNYZIK: állítsd be a GEO_GSC_OAUTH_CLIENT_JSON és GEO_GSC_OAUTH_TOKEN_JSON változókat a .env fájlban.");
  process.exit(1);
}
...
console.log("Dandelion GEO Search Console OAuth token létrehozás");
console.log("");
console.log("Nyisd meg ezt az URL-t böngészőben, jelentkezz be azzal a Google fiókkal, amelyik látja a Search Console propertyt, majd hagyd jóvá a hozzáférést:");
...
console.log("");
console.log(`OK: Search Console OAuth token mentve ide: ${tokenPath}`);
...
function waitForOAuthCode(redirectUri) {
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer((request, response) => {
      ...
      response.end("Dandelion GEO Search Console OAuth jóváhagyás kész. Visszatérhetsz a terminálhoz.");
      server.close();
      resolve(code);
    });
